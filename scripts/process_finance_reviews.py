...
from finance import parse_finance_message  # noqa: E402
...
def load_config() -> Config:
    ...
    return Config(
        root=root,
        db_path=db_path,
        agent_cmd=os.environ.get("HERMIS_FINANCE_AGENT_CMD") or None,
    )
...
def apply_agent_result(agent_result: dict[str, Any], reviews_by_id: dict[int, dict[str, Any]]):
    resolved = []
    questions = list(agent_result.get("questions") or [])
    for item in agent_result.get("resolved") or []:
        review_id = int(item["review_id"])
        entries_text = []
        for entry in item.get("entries") or []:
            amount = entry["amount"]
            currency = entry.get("currency") or "MAD"
            description = entry["description"]
            entries_text.append(f"{amount} {currency} {description}")
        parsed = parse_finance_message("\n".join(entries_text))
        if parsed.status == "parsed" and parsed.entries:
            resolved.append((review_id, parsed.entries))
        elif review_id in reviews_by_id:
            questions.append(
                {
                    "review_id": review_id,
                    "question": f"Hermis output could not be applied: {parsed.review_reason}",
                }
            )
    return resolved, questions


def deterministic_resolve(reviews: list[dict[str, Any]]):
    resolved = []
    questions = []
    for review in reviews:
        parsed = parse_finance_message(review["raw_text"])
        if parsed.status == "parsed" and parsed.entries:
            resolved.append((review["id"], parsed.entries))
        else:
            questions.append(
                {
                    "review_id": review["id"],
                    "question": f"Could not structure finance note automatically ({parsed.review_reason}).",
                }
            )
    return resolved, questions
...
def render_report(day: str, reviews, applied, questions, agent_error: str | None, question_path: Path | None) -> str:
    ...
    if agent_error:
        lines.append(f"- Agent fallback used: {agent_error}")
    if question_path:
    ...
    lines.extend(
        [
            "",
            "## Policy",
            "- Finance Discord input is review-first; user should not need manual commands for normal use.",
